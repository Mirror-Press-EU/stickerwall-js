import { Image } from "konva/lib/shapes/Image";

export default {
  getZoomValueFromImageSize( domImg:Image, dispW:number, dispH:number ) {
    let defaultZoom = 1.00;

    if (domImg.width( ) > dispW *1.125) {
      defaultZoom /= domImg.width( ) / dispW;
    }

    if (domImg.height( ) * defaultZoom < dispH) {
      defaultZoom = 1.00 / (domImg.width( ) / dispW);
    }

    return defaultZoom;
  },
  fetchImage( imgUrl:string, callback:Function ) {
    let coverImage:any = new Image( null );
    coverImage.onload = (evt:any) => callback( coverImage, true );
    coverImage.src = imgUrl;
    /*fetch( imgUrl ).then(
      resp => callback( resp.blob, true )
    ).catch(
      err => callback( err, false )
    );*/
  },
  decoration: {
    icon: ""
  }
};